...
//       // compute the  "black-and-white"  or  "gray value"  or "intensity"  of
//       // the pixel
//       grayOutput(x, y) =
...
double correlationROI(const Image<unsigned char> &backgroundPattern,
                      const Image<unsigned char> &inputGray, int x1, int y1,
                      int x2, int y2) {
  int m, n;
  int valueA, valueB;
...
void displayValueHistory(RGBImage &outFrame, Image<double> &history,
                         double newValue, double threshold, double maxValue,
                         int windowHeight, int winX, int winY) {
  int i, x, y, ybar;
  ...
  for (x = 0; x < windowWidth; x++) {
    ...
    outFrame(winX + x,
             winY + windowHeight - windowHeight * threshold / maxValue) =
        COLOR_RGB(255, 0, 0);
  }
}
...
int main() {
  ...
  system("mkdir images/output/exer_12/sixlanes1-out");
  strcpy(outputFileFormat, "images/output/exer_12/sixlanes1-out/%04d.jpg");
  frameStart = 0;
  frameEnd = 200;
  ...
  for (f = 0; f < numFrames; f++) {
    ...
    displayValueHistory(outFrame, corrHistory, corr, corrThreshold, 1.1, 50, 10,
                        180);

    for (x = x1; x < x2; x++) // draw a green box
      for (y = y1; y < y2; y++)
        if (x == x1 || x == x2 - 1 || y == y1 || y == y2 - 1)
          outFrame(x, y) = (corr > corrThreshold) ? COLOR_RGB(0, 255, 0)
                                                  : COLOR_RGB(255, 0, 0);
