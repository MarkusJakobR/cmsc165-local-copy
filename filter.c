...
void fastIntegerMeanFilter(Image<int> &outputImage,
                           const Image<int> &inputImage, int filterWidth,
                           int filterHeight) {
...
void fastIntegerMeanFilter( Image<int> & outputImage, const Image<int> &
inputImage, int filterWidth)  { int h = inputImage.height(), w =
inputImage.width(); int x,y,sum; int area = filterWidth * filterWidth;
  Image<int> horizSum;
  outputImage.resize( w,h );
  outputImage.setAll( 0 );
  horizSum.resize(w,h);
...
void correlationDouble(Image<double> &outputImage,
                       const Image<double> &inputImage,
  ...
  for (x = 0; x <= wd - patWidth; x++) {
    for (y = 0; y <= ht - patHeight; y++) {
      ...
      corr = outputImage(x + halfWidth, y + halfHeight) =
          (sumAB - sumA * sumB / area) / area /
          sqrt((sumA2 - sumA * sumA / area) / area *
               (sumB2 - sumB * sumB / area) / area);
      if (corr > maxCorr)
        maxCorr = corr;
      if (corr < minCorr)
        minCorr = corr;
...
void histogramEqualize(Image<unsigned char> &histeq,
                       const Image<unsigned char> &gray, int startX, int startY,
                       int endX, int endY, int percent) {
  ...
  numSamples =
      (endX - startX) *
      (endY - startY); // # of samples that contributed to the histogram
  histSum = 0;
  for (h = 0; h < 256; h++) { // for all unique h value
    histSum += hist[h];       // cumulative sum of the unique intensity values
    Ymap[h] = histSum * 255 / numSamples; // lookup table with values
    // gets the cumulative probability, spreading out the most frequent
    // intensity values over a wider range, increasing the overall contrast
  }
...
void stretchGrayHistogram(Image<unsigned char> &grayOutputImage,
                          const Image<unsigned char> &grayInputImage,
                          float blackPercentile, float whitePercentile) {
  Image<int> histogram;
  ...
  map.resize(256, 1);
  for (sum = 0, bin = 0; sum < (int)(blackPercentile * npix) && bin < 256;
       bin++)
    sum += histogram[bin];
  blackGray = bin;
  for (sum = 0, bin = 0; sum < (int)(whitePercentile * npix) && bin < 256;
       bin++)
    sum += histogram[bin];
  whiteGray = bin;
  for (bin = 0; bin < 256; bin++) {
    if (bin < blackGray)
      ...
      // blackGray is mapped to zero, whiteGray is mapped to 255, everything in
      // between is spread between 0..255
