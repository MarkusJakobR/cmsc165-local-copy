...
void binaryGetFirstPixel(int &xFirst, int &yFirst,
...
int binaryFollowBoundary(RGBImage &boundaryRGB, float &perimeter,
                         const Image<unsigned char> &img, int startX,
                         int startY) {
  ...
  while (!stop) {
    ...
    if (xn < 0 || xn >= width || yn < 0 || yn >= height || img(xn, yn) == 0) {
      ...
      boundaryRGB(xn, yn) =
          COLOR_RGB(255, 0, 0); // color the found boundary point
      perimeter += sqrt((xb - xn) * (xb - xn) + (yb - yn) * (yb - yn));
      xb = xn;
      yb = yn;
      nb = (nb + 4 + 1) %
           8; // exchange position and advance by one step clockwise
      xn = xb + eightN(nb, 0); //  x-neighbor
      yn = yb + eightN(nb, 1); // y-neighbor
    }
  }
...
void binaryMedianFilter(Image<unsigned char> &outputImage,
                        const Image<unsigned char> &inputImage, int filterWidth,
                        int filterHeight) {
  ...
  for (x = 0; x < w - filterWidth; x++) { // compute vertical sums
    if (x == 0) {
      ...
      for (y = 0; y < h; y++)
        horizSum[y] = horizSum[y] - inputImage(x - 1, y) +
                      inputImage(x + filterWidth - 1, y);
    }
    for (sum = 0, y = 0; y < filterHeight; y++) // the top window
      sum += horizSum[y];
    outputImage(x + filterWidth / 2, filterHeight / 2) =
        (sum > halfArea ? 1 : 0);
    for (y = 1; y < h - filterHeight; y++) { // window slides down
      sum = sum - horizSum[y - 1] + horizSum[y + filterWidth - 1];
      outputImage(x + filterWidth / 2, y + filterWidth / 2) =
          (sum > halfArea ? 1 : 0);
    }
  }
}
...
void binaryMedianFilter( Image<unsigned char> & outputImage, const
Image<unsigned char> & inputImage, int filterWidth)  { int h =
inputImage.height(), w = inputImage.width(); int x,y; int sum; int halfArea =
filterWidth * filterWidth / 2; Image<int> horizSum; outputImage.resize( w,h );
  outputImage.setAll( 0 );
  horizSum.resize(w,h);
  horizSum.setAll(0);
  for (y = 0; y < h; y++) {    // compute horizontal sums
    ...
    for (x = 1; x < w-filterWidth; x++)
      horizSum(x,y) = horizSum(x-1,y) - inputImage(x-1,y) +
inputImage(x+filterWidth-1,y);
  }
  for (x = 0; x < w-filterWidth; x++) {   // compute vertical sums
    ...
    for (y = 1; y < h-filterWidth; y++) {
      ...
      outputImage(x + filterWidth/2,y + filterWidth/2) = (sum > halfArea ? 1 :
0);
    }
  }
}
...
void binarizeGrayImage(Image<unsigned char> &outputImage,
                       const Image<unsigned char> &inputImage, int method,
...
void binaryCenterOfMass(int &xc, int &yc,
                        const Image<unsigned char> &binaryImage) {
  int sumx, sumy, x, y, n;
...
// -----------------------------------------------------------------------
void binaryErosion(Image<unsigned char> &outputImage,
                   const Image<unsigned char> &img,
                   const Image<unsigned char> &strucElem, int originX,
                   int originY) {
  int h = img.height(), w = img.width(), sh = strucElem.height(),
      sw = strucElem.width(), x, y, m, n;
  unsigned char match;

  // creates an all zero matrix for the other binary output
  outputImage.resize(w, h);
  outputImage.setAll(0);

  // iterates through all cells, does not go beyond the image dimension
  for (x = 0; x < w - sw + 1; x++)
    for (y = 0; y < h - sh + 1; y++) {
      // sets match to 1 where it determines if the strucElem has all foreground
      // (1) or has at least one background (0)
      match = 1;
      // checks if match is 1, if not then the loop ends and move to the next
      for (m = 0; m < sw && match; m++)
        for (n = 0; n < sh && match; n++)
          // if the pixel inside the strucElem is 0 or a background, set match
          // to 0 this means that there is at least one pixel inside the square
          // that is a background
          if (strucElem(m, n) && !img(x + m, y + n))
            match = 0;
      // depending on the value of match assign its value to the current origin,
      // 1 if its an object, 0 if background
...
// -----------------------------------------------------------------------
// stamps a window (structElem) to fill out gaps in the image if there exists a
// pixel in the center of the stamp
void binaryDilation(Image<unsigned char> &outputImage,
                    const Image<unsigned char> &img,
                    const Image<unsigned char> &strucElem, int originX,
                    int originY) {
  int h = img.height(), w = img.width(), sh = strucElem.height(),
  ...
  // creates an all zero matrix for the binary output
  outputImage.resize(w, h);
  outputImage.setAll(0);

  // iterate through all cells, checks that the strucElem does not go past the
  // width or height of the image
  for (x = 0; x < w - sw + 1; x++)
    for (y = 0; y < h - sh + 1; y++) {
      // checks if the current center of the strucElem is an object
      if (img(x + originX, y + originY)) {
        // iterates through the strucElem and stamps a 1, meaning the whole
        // square gets a "this is an object"
        for (m = 0; m < sw; m++)
          for (n = 0; n < sh; n++)
            if (strucElem(m, n))
              outputImage(x + m, y + n) = 1;
      }
    }
}

// -----------------------------------------------------------------------
// instead of perfect fitting, binaryLooseErosion applies a statistical
// proportion "proportionMatch" (ex. 95%) to test whether the structuring
// element fits in the current location
// -----------------------------------------------------------------------
void binaryLooseErosion(Image<unsigned char> &outputImage,
                        const Image<unsigned char> &inputImage,
                        const Image<unsigned char> &strucElem, int originX,
                        int originY, float proportionMatch) {
  int h = inputImage.height(), w = inputImage.width(), sh = strucElem.height(),
      sw = strucElem.width(), x, y, m, n;
  int numEmpty;
  ...
  for (x = 0, strucArea = 0; x < sh * sw; x++)
    strucArea += strucElem[x]; // area of strucElem
  ...
  for (x = 0; x < w - sw - 2; x++) {
    for (y = 0; y < h - sh - 2; y++) {
      ...
      for (m = 0; m < sw && match; m++) {
        for (n = 0; n < sh && match; n++) {
          if (strucElem(m, n) && !inputImage(x + m, y + n))
            numEmpty++;
...
// -----------------------------------------------------------------------
//  - Make a binary image with a particular shape.  This is useful for making
//  structuring elements.
//  - shapeType = 0 for square, 1 for circle
//  - For making structuring elements, "width" should be an odd number.
// -----------------------------------------------------------------------
void binaryMakeShape(Image<unsigned char> &outputImage, int shapeType,
                     int width) {
...
// given the binary image "inputImage", compute the two eigenvalues and
// eigenvectors from the covariance matrix
void eigenvectors2D(float &ev1, float &ex1, float &ey1, float &ev2, float &ex2,
                    float &ey2, Image<unsigned char> &inputImage) {
  ...
  c = ssxy;
  d = ssy;
  // eigenvalue computation found in
  // http://en.wikipedia.org/wiki/Eigenvalue_algorithm
  common = sqrt(4.0 * b * c + (a - d) * (a - d)) / 2.0;
  ev1 = (a + d) / 2.0 +
        common; // first eigenvalue (ev1) and a sample eigenvector (ex1,ey1)
  ex1 = 1.0;
  ey1 = (ev1 - a + c) * ex1 / (ev1 + b - d);
  ev2 = (a + d) / 2.0 -
        common; // second eigenvalue (ev2) and a sample eigenvector (ex2,ey2)
  ex2 = 1.0;
  ey2 = (ev2 - a + c) * ex2 / (ev2 + b - d);
}

// return the two points on the binary image that are farthest from each other
void farthestPairOfPoints(int &x1max, int &y1max, int &x2max, int &y2max,
                          int &length, Image<unsigned char> &binary) {
...
int ConnectedComponents::analyzeBinary(const Image<unsigned char> &binary,
      ...
      while (startQ != endQ) {
        ...
        for (s = startQ; s != currentEndQ; s = (s + 1) % qLimit) {
          ...
          for (c = 0; c < numConnect; c++) {
            cx = x + connect(0, c);
            cy = y + connect(1, c);
            if (0 <= cx && cx < w && 0 <= cy && cy < h && binary(cx, cy) &&
                mTagImage(cx, cy) == -1) {
              mTagImage(cx, cy) = tag;
              queue[endQ] = cy * w + cx;
              endQ = (endQ + 1) % qLimit;
  ...
  for (tag = 0; tag < mNumComponents; tag++) {
    mBbox(tag, 2) =
        mBbox(tag, 2) - mBbox(tag, 0) + 1; // max x is changed to bbox width
    mBbox(tag, 3) =
        mBbox(tag, 3) - mBbox(tag, 1) + 1; // max y is changed to bbox height
  }
...
Image<unsigned char> ConnectedComponents::getComponentBinary(int n) {
  if (n >= getNumComponents() || n < 0) {
    printf("ConnectedComponents::getComponentBinary : n(%d) is not in range "
           "0..%d\n",
  ...
  Image<unsigned char> outimg;
  int xstart = mBbox(n, 0), ystart = mBbox(n, 1);
  int w = mBbox(n, 2), h = mBbox(n, 3);
  int x, y;
  outimg.resize(w, h);
  outimg.setAll(0);
  ...
  for (x = 0; x < w; x++)
    for (y = 0; y < h; y++)
      if (mTagImage(xstart + x, ystart + y) == n)
        outimg(x, y) = 1;
  return outimg;
}
...
RGBImage ConnectedComponents::randomColors() {
  if (getNumComponents() == 0) {
    printf("ConnectedComponents::randomColors : Cannot create connected "
           "component color image (mNumComponents == 0)\n");
