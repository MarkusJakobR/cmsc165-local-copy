...
int main(int argc, char *argv[]) {
  ...
  for (iter = 0; iter < numIterations; iter++) {
    ...
    for (p = 0; p < npix; p++) {
      pix = inputImage[p];
      cMin = 0;
      minDist = square(RED(pix) - mean(cMin, 0)) +
                square(GREEN(pix) - mean(cMin, 1)) +
                square(BLUE(pix) - mean(cMin, 2)); // distance from mean 0
      // find the nearest mean
      for (c = 1; c < K; c++) {
        dist = square(RED(pix) - mean(c, 0)) + square(GREEN(pix) - mean(c, 1)) +
               square(BLUE(pix) - mean(c, 2));
        if (dist < minDist) {
          cMin = c;
          minDist = dist;
        }
  ...
  sprintf(folderName, "images/output/exer_11/%s_%d_kmeans", extractedFilename,
          K);
  ...
  for (c = 0; c < K; c++) { // for each cluster
    ...
    for (x = 1; x < width - 1; x++) {
      for (y = 1; y < height - 1; y++) {
        ...
        if (label(x, y) ==
            c) { // copy the color, plus do cross-shaped binary dilation
    ...
    sprintf(fileName, "images/output/exer_11/%s_%d_kmeans/%04d_%s.jpg",
            extractedFilename, K, c, extractedFilename);
