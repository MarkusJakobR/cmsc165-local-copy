...
int main() {
  ...
  system("mkdir images/output/exer_12/backsub");
  strcpy(inputFileFormat, "images/student/student%04d.jpg");
  strcpy(outputFileFormat, "images/output/exer_12/backsub/backsub%04d.jpg");
  ...
  for (x = 0; x < width; x++) { // for every pixel  (x,y)
    for (y = 0; y < height; y++) {
      ...
      outputFrame(x, y) =
          COLOR_RGB(background(x, y), background(x, y), background(x, y));
    }
  }
  ...
  for (f = 0; f < numFrames; f++) {
    ...
    // write the output image as  input image beside the binary image (result of
    // background subtraction)
    outputFrame.resize(width * 2, height);
    for (x = 0; x < width; x++) {
      for (y = 0; y < height; y++) {
        outputFrame(x, y) =
            inputFrame(x, y); // left side contains the original image
        if (binary2(x, y) == 1)
