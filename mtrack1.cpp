...
int main(int argc, char *argv[]) {
  ...
  RGBImage currentFrame, prevFrame, prevFrame2, prevFrame3, outFrame;
  ...
  for (f = 0; f < numFrames; f++) {
    ...
    if (f > 2) {
      ...
      for (x = 0; x < width; x++)
        for (y = 0; y < height; y++) {
          ...
          motion(x, y) = weight = abs(RED(pix) - RED(pix3)) +
                                  abs(GREEN(pix) - GREEN(pix3)) +
                                  abs(BLUE(pix) - BLUE(pix3));
      ...
      for (y = ytp - vicHalfHeight; y < ytp + vicHalfHeight; y++) {
        if (y >= 0 && y < height && xtp - vicHalfWidth >= 0 &&
            xtp + vicHalfWidth < width) {
      ...
      for (x = xtp - vicHalfWidth; x < xtp + vicHalfWidth; x++) {
        if (x >= 0 && x < width && ytp - vicHalfHeight >= 0 &&
            ytp + vicHalfHeight < height) {
  ...
  for (f = 0; f < numFrames; f++) {
    ...
    currentFrame(trajectory(f, 0) + 1, trajectory(f, 1)) =
        COLOR_RGB(255, 128, 0);
    currentFrame(trajectory(f, 0), trajectory(f, 1) + 1) =
        COLOR_RGB(255, 128, 0);
    currentFrame(trajectory(f, 0) + 1, trajectory(f, 1) + 1) =
        COLOR_RGB(255, 128, 0);
  }
  writeJpeg(currentFrame, "images/output/exer_12/track/traject.jpg", 90);
}
