...
template <class T>
Image<T>::Image(
    const Image<T> &img) { // important for functions returning Image
...
template <class T> void Image<T>::resize(int w, int h) {
  int i;
  delete[] pixel;
...
template <class T> void Image<T>::setAll(T x) {
  int n = width() * height();
  for (int i = 0; i < n; i++)
    pixel[i] = x;
}
...
public:
  RGBImage() {};
  ~RGBImage() {};
  inline void setPix(int x, int y, unsigned char r, unsigned char g,
                     unsigned char b) {
