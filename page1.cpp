...
int main(int argc, char *argv[]) {
  ...
  int strucWidth, strucHeight; // dimensions of structuring element for cleaning
                               // and smearing
  int minimumArea =
      10; // minimum size of objects to be considered, anything smaller is noise
  ...
  sprintf(folderName, "images/output/exer_8/page/%s_page", extractedFilename);
  ...
  for (group = 0; group < 4; group++) {
    ...
    // find all connected components.  Connectedness can be specified by
    // FOUR_CONNECTED or EIGHT_CONNECTED
    cc.analyzeBinary(binary2, EIGHT_CONNECTED);

    outputImage =
        cc.randomColors(); // show each component with a different color
    sprintf(fileName, "%s/%s_object-pieces-%d.jpg", folderName,
            extractedFilename, group);
    ...
    sprintf(fileName, "%s/%s_object-group-%d.jpg", folderName,
            extractedFilename, group);
    writeJpeg(outputImage, fileName, 100);
  }
}
