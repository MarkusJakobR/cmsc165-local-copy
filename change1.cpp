...
int main(int argc, char *argv[]) {
  ...
  readJpeg(inputImage1, argv[1]); // scene1.jpg
  readJpeg(inputImage2, argv[2]); // change4.jpg
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++) {
      ...
      // a good alternaative could be to use tthe euclidean distance between
  ...
  sprintf(filename, "images/output/exer_12/change/%s_%03d_changes.jpg",
          extractedFilename, threshold);
