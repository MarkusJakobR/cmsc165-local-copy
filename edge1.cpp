...
/*
   Edge detection using
      ...
      2. The image gradient (first derivative) and zero-crossing
  (second-derivative) The program reads in a color JPEG image, and highlights
...
int main(int argc, char *argv[]) {
  ...
  char edgeOutputFilename[100];
  char zeroOutputFilename[100];
  char gradientOutputFilename[100];
  ...
  // default = 50
  double gradientMagnitudeThreshold = atoi(argv[2]);

  // read the JPEG image
  readJpeg(img, argv[1]);
  width = img.width();
  height = img.height();
  edgeVisual.resize(width,
                    height); // RGB visualization of the significant edges
  gradientVisual.resize(width, height); // RGB visualization of the gradient
  zeroCrossVisual.resize(width,
                         height); // RGB visualization of the zero crossings
  ...
  for (x = 0; x < width; x++)
    for (y = 0; y < height; y++)
      grayScale(x, y) =
          (RED(img(x, y)) + GREEN(img(x, y)) + BLUE(img(x, y))) / 3;
  ...
  // compute the second derivative image by convolving filteredIntensity with
  // Laplacian operator
  ...
  for (x = 0; x < width - 1; x++) {
    for (y = 0; y < height - 1; y++) {
      gx = gradientX(x, y);
      gy = gradientY(x, y);
      gradient = pow(pow(gx, 2) + pow(gy, 2),
                     0.5); // magnitude of the gradient (gx,gy)
      gradientGray = min(255, (int)gradient);
      gradientVisual(x, y) =
          COLOR_RGB(gradientGray, gradientGray, gradientGray);

      edgeVisual(x, y) = COLOR_RGB(RED(img(x, y)) / 3, GREEN(img(x, y)) / 3,
                                   BLUE(img(x, y)) / 3);
      zeroCrossVisual(x, y) = edgeVisual(x, y);
      // check if its a significant edge:   a high first-derivative gradient
      // magnitude
  ...
  sprintf(edgeOutputFilename, "images/output/exer_5/edge_%.2f.jpg",
          gradientMagnitudeThreshold);
  sprintf(zeroOutputFilename, "images/output/exer_5/zero_%.2f.jpg",
          gradientMagnitudeThreshold);
  sprintf(gradientOutputFilename, "images/output/exer_5/gradient_%.2f.jpg",
          gradientMagnitudeThreshold);

  // write the visualization of the edges
  writeJpeg(edgeVisual, edgeOutputFilename, 100);
  writeJpeg(zeroCrossVisual, zeroOutputFilename, 100);
  writeJpeg(gradientVisual, gradientOutputFilename, 100);
}
