...
int main(int argc, char *argv[]) {
  ...
  int strucWidth = atoi(argv[3]);  // width of structuring element for cleaning
  int minimumArea = atoi(argv[4]); // range of size of objects to be considered
  int maximumArea = atoi(argv[5]);
  float perimeter;
  int intensityThreshold = atoi(argv[2]); // threshold for binarizing
  ...
  sprintf(folderName,
          "images/output/exer_8/boundary/%s_%03d_%03d_%03d_%03d_boundary",
  ...
  sprintf(
      outputFilename,
      "images/output/exer_8/boundary/%s_%03d_%03d_%03d_%03d_object-pieces.jpg",
      extractedFilename, intensityThreshold, strucWidth, minimumArea,
      maximumArea);
  ...
  for (c = 0; c < cc.getNumComponents(); c++) {
    ...
    if (minimumArea <= numPix && numPix <= maximumArea) {
      ...
      for (y = startY; y <= startY + ch; y++) {
        outputImage(startX, y) = outputImage(startX + 1, y + 1) =
            pixColor; // make the line 2-pixel thick
        outputImage(startX + cw, y) = outputImage(startX + cw + 1, y + 1) =
            pixColor; // make the line 2-pixel thick
      }
      // draw the top and bottom boundaries
      for (x = startX; x <= startX + cw; x++) {
        outputImage(x, startY) = outputImage(x + 1, startY + 1) =
            pixColor; // make the line 2-pixel thick
        outputImage(x, startY + ch) = outputImage(x + 1, startY + ch + 1) =
            pixColor; // make the line 2-pixel thick
      }
      ...
      binaryFollowBoundary(boundaryRGB, perimeter, componentImage, boundStartX,
                           boundStartY);
      ...
      sprintf(fileName, "%s/%s_%03d.jpg", folderName, extractedFilename, c);
  ...
  sprintf(outputFilename,
          "images/output/exer_8/boundary/"
          "%s_%03d_%03d_%03d_%03d_objects-selected.jpg",
          extractedFilename, intensityThreshold, strucWidth, minimumArea,
          maximumArea);
  ...
  printf("\nThere are %d components in the binary image. \n",
         cc.getNumComponents());
  printf("See the image boundary/object-pieces.jpg  for a visualization of the "
         "components.\n");
}
