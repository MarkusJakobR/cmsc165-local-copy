...
//========================================================================
void convertSingleChannelJPEG(Image<unsigned char> &gray,
                              const RGBImage &inputFrame) {
...
int cropImageRGB(RGBImage &outputImage, const RGBImage &inputImage, int x1,
                 int y1, int x2, int y2) {
  int x, y;
  if (x1 >= x2 || y1 >= y2 || x1 < 0 || y1 < 0 || x2 >= inputImage.width() ||
...
int concatImageRGB(RGBImage &outputImage, const RGBImage &inputImage1,
                   const RGBImage &inputImage2) {
  int x, y;
...
// NEW FUNCTION
// takes the left and right half of each image and placing them in a single
// image with the same size
int overlayHalfImage(RGBImage &outputImage, const RGBImage &inputImage1,
                     const RGBImage &inputImage2) {
  int x, y;
  int width1 = inputImage1.width(), height1 = inputImage1.height();
  int width2 = inputImage2.width(), height2 = inputImage2.height();

  outputImage.resize(max(width1, width2), max(height1, height2));
  outputImage.setAll(COLOR_RGB(255, 255, 255));
...
//========================================================================
void translateRGB(RGBImage &shiftedRGB, const RGBImage &img, int translateX,
                  int translateY) {
  int width = img.width();
...
void rotateRGB(RGBImage &outputImage, const RGBImage &inputImage,
  ...
  for (xp = 0; xp < width; xp++) {
    for (yp = 0; yp < height; yp++) {
      ...
      //      xs = max(0,min(width-1,xs));  // force the source location to
      //      within image bounds ys = max(0,min(height-1,ys));
...
//========================================================================
void verticalProjection(Image<int> &vertProjection,
...
void overlayRuler(RGBImage &outputImageRGB, int yStart, int pixPerMM) {
  ...
  int color[10];
  int c, x, m, n;
  color[0] = COLOR_RGB(255, 255, 255);
  color[1] = COLOR_RGB(192, 0, 0);
  color[2] = COLOR_RGB(0, 192, 0);
  color[3] = COLOR_RGB(0, 0, 192);
  color[4] = COLOR_RGB(255, 255, 0);
  color[5] = COLOR_RGB(0, 255, 255);
  color[6] = COLOR_RGB(255, 128, 128);
  color[7] = COLOR_RGB(128, 255, 128);
  color[8] = COLOR_RGB(128, 128, 255);
  color[9] = COLOR_RGB(120, 120, 0);
  x = xStart;
...
void overlayImageRGB(RGBImage &outputImageRGB, const RGBImage &inputImageRGB,
                     int x0, int y0) {
  int x, y;
...
void drawBoxRGB(RGBImage &inputImageRGB, int x1, int y1, int x2, int y2,
...
void drawLineRGB(RGBImage &inputImageRGB, int x1, int y1, int x2, int y2,
                 int color, int thickness) {
  ...
  if (abs(x2 - x1) > abs(y2 - y1)) {
    ...
    for (x = x1; x != x2; x += sign) {
      ...
      for (m = -half; m < half; m++) {
        if (inputImageRGB.isInside(x + m, y))
          inputImageRGB(x + m, y) = color;
        if (inputImageRGB.isInside(x, y + m))
          inputImageRGB(x, y + m) = color;
      }
    }
  } else {
...
void makeGrayFromRGB(Image<unsigned char> &grayOutput,
  ...
  grayOutput.resize(
      width,
      height); // output image has same dimensions as the input color image
  npix = width * height;
  ...
  for (p = 0; p < npix; p++) {
    ...
    // compute the  "black-and-white"  or  "gray value"  or "intensity"  of the
    // pixel
    grayOutput[p] =
        (red + green + blue) / 3; // average of the red, green and blue
  }
} // end of function

void makeRGBFromGray(RGBImage &colorOutputRGB,
                     Image<unsigned char> &grayImage) {
  int npix, p;
  int width = grayImage.width();
  int height = grayImage.height();
  colorOutputRGB.resize(
      width, height); // color output has same dimensions as the input image
  npix = width * height;
  //  visit every pixel in the input image
  for (p = 0; p < npix; p++) {
...
void makeRGBFromBinary(RGBImage &colorOutputRGB, Image<unsigned char> &binary1,
                       int method, int scale) {
  int i, x, y, pix;
  ...
  case 4: // method == 4   [0 == gray, 1 == original+striped+boundary outline]
    for (x = 1; x < width - 2; x++) {
      for (y = 1; y < height - 2; y++) {
        if (binary1(x, y) != binary1(x + 1, y) ||
            binary1(x, y) != binary1(x, y + 1) ||
            (binary1(x, y) && y % 10 == 0)) {
          colorOutputRGB(scale * x, scale * y - 1) = red;
          colorOutputRGB(scale * x - 1, scale * y) =
              colorOutputRGB(scale * x, scale * y) =
                  colorOutputRGB(scale * x + 1, scale * y) = red;
          colorOutputRGB(scale * x, scale * y + 1) = red;
        }
      }
    }
    ...
    for (x = 0; x < width; x++) {
      for (y = 0; y < height; y++) {
        if (binary1(x, y) == 0) {
          ...
          colorOutputRGB(scale * x, scale * y) =
              COLOR_RGB(RED(pix) / 2, GREEN(pix) / 2, BLUE(pix) / 2);
        }
      }
    }
    break;
